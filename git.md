...
版本回退：
        ...
        2)已提交commit： git reset --hard HEAD~[i]  回退到该状态===『之后的提交commit-id都没有了』
                         git reset --soft HEAD~[i]  回退（之后的变化处在暂存区，commit即可,但提交亦无）
        3)特殊：    git revert [HEAD~/commid-id]   把该提交重新做一次（像是先回到当前状态，后commit）
前进：回退多了
