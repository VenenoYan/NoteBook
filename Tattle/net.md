...
第一次：客户端告诉服务器我要和你通信，同时同步序列号为你可以用该序列段起始地址回应我。（这些字段的作用是这样的：通过SYN=1，ACK=0告诉server这是连接请求Connection Requet。SEQ=x表明后面传送数据时的第一个数据字节的序号是x+1）
  * 
第二次：服务器说，好的，你可以发送数据了；用这个序列号作为起始数据段来回应我；(server通过SYN=1，ACK=1告诉client，这是接受请求Connection Accepted。确认你是要从x+1传起了。server也通知client，我要传给你的是从y+1传起的哦！）
  * 
第三次：客户端说，哦好的，我知道你准备好了，那么接下来我发送数据了哈！（但是第三次理论上可以带数据，没有SYN标志即可，client告诉server，OK，我是要建立连接，你可以建立了）
...
网络号+主机号 或者 网络前缀+主机号
    * 
比如：185.46.54.45/24。表示24位的网络前缀，12位主机地址====子网掩码就是255.255.255.0
    * 
子网号是从网络号借来的；子网掩码就是网络号+子网号均是1，主机号为0，两个IP的掩码就是
* 
全0是本机地址，全1是广播地址，主机号全0是网络本身
* 
网络前缀相同的连续IP可以路由组合或称超网
