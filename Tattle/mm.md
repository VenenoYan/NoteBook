...
* ###三、STL
    ...
    * map为什么用红黑树而不是AVL：
        ...
        * 因为 AVL 树是高度平衡的树, 而每一次对树的修改, rebalance的可能要比红黑树高的多, 这里的开销会比红黑树大. 红黑树插入只要两次旋转和AVL相同,O(1)复杂度 删除至多三次旋转，但是AVL可能多次从被删节点一直到根节点. 但不可否认的是, AVL 树搜索的效率是非常稳定的. 选取红黑树, 是一种折中的方案.
