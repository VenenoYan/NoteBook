...
###一、C++
    ...
    * 宏一般的用处、不可被const取代的方面：
        * 宏：整个程序内替换，编译期完成没有类型检查，文本替换
        * const：当前文件替换，有内存，有类型检查
    * 上述两者各有那些优缺点：
        * 宏不会做安全检查及类型检查
        * const有类型检查，更可靠
    * int \*p[]与int (*p)[]的区别：
...
* ###三、STL
    * vector实现以及删除中间的元素
        * 底层使用数组；可以随机访问元素，尾后插入比中间块
        * 添加和删除中间的元素都是导致之后的元素移动
        * 添加：返回第一个添加成功元素的迭代器。可能所有迭代器失效
        * 删除：返回删除成功后第一个元素迭代器。只有后面的迭代器失效
    * map实现机制
    * map为什么用红黑树而不是AVL：
        * 需要考虑的是：查找search、插入insert、删除delete的效率
        * 因为 AVL 树是高度平衡的树, 而每一次对树的修改, rebalance的可能要比红黑树高的多, 这里的开销会比红黑树大. 红黑树插入只要两次旋转和AVL相同, 删除至多三次旋转，但是AVL可能多次从被删节点一直到根节点. 但不可否认的是, AVL 树搜索的效率是非常稳定的. 选取红黑树, 是一种折中的方案.
