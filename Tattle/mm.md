...
必须由程序员在程序中显式地指定。
    ...
    * 重载函数怎么实现？    
        * C++编译时，产生的符号表内部函数名包含参数（define不进入符号表）
    * inline关键字的作用? 有什么缺点?
    ...
    * [虚函数、虚函数表](./../C++/class.md)：
        * 虚函数是怎么实现的：
            * 为了多态；通过父类的指针或者引用指向子类的对象，调用函数时通过查找虚函数表从而决定调用那个函数  
    * 虚基类（接口）：
    * 普通继承、多重继承：
    * 菱形继承、虚继承：
    * explicit的作用：
    ...
    * 宏一般的用处、不可被const取代的方面、typedef：
        * 宏：整个程序内替换，编译前完成，**没有类型检查**，文本替换=====不分配内存，编译
            * 当**出错时**，由于**没有进入符号表，**所以提示后面的143而不是前面的PPP。```#define PPP 143```
...
（4）h &= ~x;                表示把h的高4位清零。```
    ...
    * 有10个文件，每个文件1G，每个文件的每一行存放的都是用户的query，每个文件的query都可能重复。要求你按照query的频度排序。 
        ...
        * 统计每个小文件中query的出现情况；
        * 对所有小文件的统计情况排序
    * 40亿个不重复unsigned int。给出几个数判断是否出现过。
        * 用bitmap。
    ...
    * 只扫描一遍如何判断一副牌是否胡了？？
        * 一般胡牌：三个加一个对
            ...
            * 最后遍历所有，只要该项值为3即可。
    * 设计一个带权的随机函数
        * 权重相加为1，那么根据随机函数产生0~1之间的随机数，按照随机值对应的百分比进行输出即可
    * 从n的集合里面取出m个进行组合、排列
        * 排列：可以将m个分为第一个和剩下的，然后剩下的也分为第一个和剩下的、、、
        * 组合：声明一个数组，记录0/1（未选中、选中），然后对于所有的10，换成01
    * 直接公式：
        * 合法的入栈出栈序列有多少个、二叉树的枚举、多边形分成三角形的个数、圆括弧插入公式中的方法数，这就是catalan数其通项是c(2n, n)/(n+1)。
* ###六、操作系统：
...
原理：
    ...
    * 进程和线程、协程切换的差异，各自的上下文包括什么
        * 进程：保存寄存器、程序计数器、堆栈指针、更新状态。恢复时在反向来一遍。所以有两次模式切换：用户-内核、内核-用户
        * 线程：无法利用多核
            * 用户级：只需要保存用户寄存器的内容，程序计数器、寄存器，栈指针，不需要模式切换。
            * 内核级：内核完成，用户没有权限
    ...
    * 死锁
        ...
        * 处理：
            ...
            * 监测：
            * 解决：
    * 中断：CPU在执行过程中由于某些需要，终止当前的运行过程去处理其他任务的过程；
        * 关中断
        * 保护现场
        * 处理
        * 恢复现场
        * 开中断
        * 正在处理中断时，如果一个新的更高优先级的中断来了，那么会抢占中断
    * 为什么需要内存对齐.
...
为什么内存快，外存慢
    * 
因为离得远？当然不是，因为：外存是物理机械器件，而内存是存储单元构成的电路，靠脉冲信号工作，而不是外存靠读写头加盘片工作的。
