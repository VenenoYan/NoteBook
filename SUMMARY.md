...
**注：**
模板不能把声明和定义分开，这是因为链接的时候出会错。C++标准规定，模板在不被使用的时候不应该实例化，所以虽然我们在H文件中声明了一个模板函数，但是CPP中并没有使用过即永远不会实例化，所以也就永远不会像普通函数一样被找到并使用。在实例化的程序中，由于声明定义分开，那么比如```X<char>.foo()```，调用foo函数，由于.H中是声明，所以需要去其他文件中找，找到CPP文件的目标文件，但是没有```X<char>.foo()```的定义(没有该实例的定义，只有模板)。**注意**如果我们的.CPP文件中有```X<char>.foo()```的使用例子，那么就可以找到，因为使用了编译时就会实例化，然后我们就可以找到。
