...
###3.5 最短路径：Dijkstra迪杰斯特拉
####3.5.1 从某个原点到其他顶点的最短路径：单源最短路径
* 
对该顶点建立一个辅助数组D，记录从该顶点到其余各顶点的距离：初始值为两顶点间有弧则为权值，否则∞；<br>新建S代表已经得到的最短距离的集合，初始为空；
* 
在V-S的顶点集合中：从数组D中找到最小值的一个顶点u，即当前求得的一条从该顶点出发的最短路径的终点，加入S中
* 
更新其余的值：对于V-S中的其他顶点，如果**D[u] + arcs[s][u]<D[k],则更新D[k]=D[u] + arcs[s][u]**
* 
